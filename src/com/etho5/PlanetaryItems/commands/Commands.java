                        ...
                        if (args[2].equalsIgnoreCase(items.toString())) {
                            try {
                                if (Integer.parseInt(args[3]) > 0) {
                                    ...
                                    pl.getInventory().addItem(worldItem);
                                    pl.updateInventory();
                                    worldItem.setAmount(1);
                                    return true;
                                }
                            } catch (NumberFormatException exception) {
                                p.sendMessage(ChatColor.DARK_GREEN + "[" + ChatColor.DARK_PURPLE + "PlanetaryItems" + ChatColor.DARK_GREEN + "] Please give a number amount!");
                            }
                            break;
                        }
                    }
                } else p.sendMessage(ChatColor.DARK_GREEN + "[" + ChatColor.DARK_PURPLE + "PlanetaryItems" + ChatColor.DARK_GREEN + "] Please give an online player!");
                break;
            }
        }
